...
var BRAVE_BASE_URL = process.env.BRAVE_BASE_URL || "https://api.search.brave.com";
var BRAVE_SEARCH_URL = `${BRAVE_BASE_URL}/res/v1/web/search`;
...
function getContext() {
  ...
  if (recent.length) {
    ...
    for (const c2 of recent) {
      parts.push(`[${c2.timestamp}] Query: ${c2.user.slice(0, 150)}`);
...
function searchMemory(query) {
  ...
  for (const c2 of getRecentConversations(50)) {
    if ((c2.user + c2.assistant).toLowerCase().includes(q2)) {
      results.push(`[${c2.timestamp}] Query: ${c2.user.slice(0, 100)} \u2192 Result: ${c2.assistant.slice(0, 100)}`);
    }
  }
  const facts = loadFacts();
...
var tools = [
  {
    name: "web_search",
    description: "Search the web for real-time information. Use this to find current facts, news, documentation, or any topic the user asks about. You can call this multiple times with different queries to get broader coverage.",
    input_schema: {
      type: "object",
  ...
  {
    name: "remember",
    description: "Search your memory for past research sessions, facts, and entities. Use this when the user references something from a previous session or asks what you know about a topic.",
    input_schema: {
      type: "object",
...
async function research(query, messageId) {
  ...
  const systemPrompt = `You are Web Research Agent, an expert research assistant. Today is ${today}. Your job is to thoroughly research the user's question using web search. Strategy:
...
## Memory
You have persistent memory across sessions. Previous research and facts are automatically provided below. Use the \`remember\` tool to search for specific past topics when needed.

If the user references a previous session (e.g. 'what did you find about X last time?'), check your memory context or use the remember tool.`;
  const enrichedQuery = memoryContext ? `## Memory context from past sessions
${memoryContext}
  ...
  while (true) {
    ...
    for (const block of resp.content) {
      if (block.type === "tool_use") {
        const q2 = block.input?.query || "";
        send({
          type: "activity",
          tool: block.name,
          description: q2 ? `${block.name}(${q2})` : block.name,
          message_id: messageId
        });
      }
    }
    if (resp.stop_reason === "end_turn" || !resp.content.some((b2) => b2.type === "tool_use")) {
      const textBlocks = resp.content.filter((b2) => b2.type === "text");
      const result = textBlocks.map((b2) => b2.text).join("") || "";
